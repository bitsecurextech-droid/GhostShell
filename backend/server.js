...
app.post('/api/admin/tool', adminAuth, async (req, res) => {
    ...
    const id = uuidv4();
    await query(`INSERT INTO tools (id, name, description, price_usd, category, download_url, payment_link, image_urls, video_url)
...
app.put('/api/admin/tool/:id', adminAuth, async (req, res) => {
    const { name, description, priceUSD, category, downloadUrl, paymentLink, imageUrls, videoUrl } = req.body;
    await query(`UPDATE tools SET name=$1, description=$2, price_usd=$3, category=$4, download_url=$5, payment_link=$6, image_urls=$7, video_url=$8 WHERE id=$9`,
...
async function seedDatabase() {
    ...
    if (parseInt(esimCount.rows[0].count) === 0) {
        await query(`INSERT INTO esim (id, name, duration, price, data, voice) VALUES ($1, $2, $3, $4, $5, $6)`,
