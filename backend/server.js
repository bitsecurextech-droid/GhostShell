...
// ------------------------------------------------------------------
// ✅ CRITICAL FIX: Initialize all required arrays BEFORE anything else
// ------------------------------------------------------------------
async function initializeDatabase() {
    ...
    // List of all expected array fields
    const requiredKeys = [
    ...
    for (const key of requiredKeys) {
        if (!db.data[key] || !Array.isArray(db.data[key])) {
            db.data[key] = [];
            console.log(`⚠️ Missing or invalid "${key}" – reset to empty array.`);
        }
    }

    await db.write();
    console.log('✅ Database ready. All required tables exist.');
}
...
// -------------------- EXTERNAL APIS (minimal – keep your full ones) --------------------
// (I'm not repeating all external APIs, but they work as before.
//  Make sure to use `if (!db.data.xxx) db.data.xxx = []` before any array access.)
...
function createAdminCrud(resourceName, dbKey) {
    ...
    app.post(`/api/admin/${resourceName}`, adminAuth, async (req, res) => {
        ...
        if (!db.data[dbKey]) db.data[dbKey] = [];
...
// Admin endpoints for users, codes, payments, tools (keep your existing ones)
...
initializeDatabase().then(async () => {
    ...
    // Seed default data if empty (optional)
